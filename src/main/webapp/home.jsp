...
<div class="shop-nav shop-index">
    ...
    <div class="shop-banner">
        ...
        <div class="layui-container layui-hide-xs">
            <div class="product-list">
                <dl id="getIndexRoomType">
                    <dt style="background-color: #009688"><a href="/room/roomList.htm" target="_blank">房间分类</a></dt>
                    <c:forEach var="roomTypes" items="${roomTypesList}">
                        <dd data-id="${roomTypes.id}">
                            <a class="fly-case-active" href="/room/roomList/${roomTypes.id}.html"
                               data-type="toRoomTypeList">${roomTypes.typename}</a>
                        </dd>
                    </c:forEach>
                </dl>
            </div>
        ...
        <div class="shop-temp" id="getIndexFloor">
            <%--遍历model模型中的楼层信息，这里需要很细心查看--%>
            <c:forEach items="${floorList}" var="floor" varStatus="status">
                <%--判断当前的下标是否为偶数行，如果是div的背景为白色，否则为灰色--%>
                <c:if test="${status.index%2==0}">
                    <div class="temp-hot"></c:if>
                <c:if test="${status.index%2!=0}">
                    <div class="temp-hot" style="background-color: #f2f2f2"></c:if>
                <div class="layui-container">
                    <p class="temp-title-cn"><span></span>酒店${floor.name}<span></span></p>
                    <div class="layui-row layui-col-space20">
                        <c:forEach var="room" items="${roomList}">
                            <%--判断楼层和房间楼层id是否相等，相等的就显示在相同的楼层中--%>
